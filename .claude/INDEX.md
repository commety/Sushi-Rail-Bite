...
세 손님 유형의 대역 배치와 근거. 유형은 **코드 분기가 아니라 데이터 차이**라는 불변식, 전문가는 좁고 범용가는 **덱 전 구간을 덮어야 한다**는 실측 결론(3분할로 좁혔다가 스테이지 1 이 클리어 불가가 된 사고), 경계값에서 폭이 승자를 정한다는 것, **판에서 유형을 무엇으로 구별하나**(M6.6 — 상시 라벨을 걷고 실루엣·상태 표시·정보 창 셋으로 나눴다).
