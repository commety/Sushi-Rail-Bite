...
- 모든 언어 UI 전수 QA — 핵심 플로우 (덱빌딩·스테이지 진행·결과 화면) + 가장 긴 언어 1개만으로도 80% 커버.
...
## 게임 특히 중요한 이유
- 웹(WebGL) 데모는 링크 하나로 해외 유저에게 바로 노출된다. 지역별 리텐션은 번역·현지화 품질에 강하게 연동. UI 깨짐은 별점·리뷰에 직결 (Ch 5 사례 Indonesia "last name" 일화 — 40% 가입 불가).
- 이 게임은 "매출(금액)" 이 곧 점수다 — 통화 기호·천 단위 구분자 포맷이 locale 따라 깨지면 핵심 UI 가 바로 망가진다.
- 문화적 이미지 (음식·손 제스처·색 의미) 는 지역마다 다르다 — Ch 5 디아퍼 스토크/복숭아 일화.
