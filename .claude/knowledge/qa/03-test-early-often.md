...
## Do
- **위험한 변경** 직후에는 바로 확인: 코어 루프(초밥 스폰·집기·매출 산정)·스테이지 클리어 판정·덱/런 상태 저장 같은 "깨지면 아픈" 것을 건드렸으면 같은 세션에서 `/qa` 로 스모크.
...
- 매 PR 에 별도 `/qa` 패스 — 여력 있을 때. (단, EditMode 테스트 동반은 커밋 게이트라 별개다 — `CLAUDE.md` §5.4·§8)
