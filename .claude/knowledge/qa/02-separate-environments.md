...
- 테스트 전용 씬(예: `Assets/Level/Scenes/QA/*.unity`) 에서 최소 초기 조건만 세팅. 개발 중인 메인 씬에 테스트 코드·상태를 섞지 않는다.
- 다른 해상도 · 타겟 framerate · input 조합에서 스모크 — WebGL 이면 데스크톱 브라우저 2종(Chrome·Safari) + 창 크기 2종.
...
- Unity Test Framework: `Tests.EditMode` (`Assets/Tests/EditMode`), `Tests.PlayMode` (`Assets/Tests/PlayMode`) 분리 — 격리 기본 구조.
