# Unity Performance — Engine-layer Knowledge (WebGL 타깃 기준)

범용 Unity 성능 규칙. 특정 프로젝트보다 상위 계층. 출처: Unity Technologies, *Optimize Your Mobile Game Performance*.

원본은 모바일 기준으로 쓰였고, 이 프로젝트의 배포 타깃은 **WebGL** 이다 (`CLAUDE.md` §1.2). 두 타깃은 제약이 거의 겹치므로 — 저사양 CPU, 제한된 메모리, GC 스파이크에 취약, 드로우콜에 민감 — 아래 규칙은 **§0 의 보정을 적용해서** 읽는다.

원칙은 단순하다: **측정하지 않고 최적화하지 않는다.** 아래 규칙은 프로파일러로 문제를 확인한 뒤 적용한다.

---

## 0. WebGL 타깃 보정 (이 프로젝트에서 다르게 읽어야 할 것)

| 항목 | 원문(모바일) | WebGL 에서는 |
|---|---|---|
| **초기 로드 크기** | 상대적으로 덜 중요 | **최우선 지표.** 브라우저는 전체 빌드를 내려받아야 시작한다. 텍스처·오디오·씬 수가 곧 이탈률 |
| **텍스처 압축** | ASTC / ETC2 / PVRTC | **DXT (DXT1/DXT5)** 가 데스크톱 브라우저의 사실상 표준. ASTC 는 지원이 고르지 않다 |
| **스레딩** | 워커 스레드 사용 가능 | **싱글 스레드.** `System.Threading`·`Task.Run` 기반 병렬화가 동작하지 않는다. 코루틴/프레임 분할로 푼다 |
| **GC** | 스파이크 주의 | 동일하되 **더 아프다.** 힙 확장 비용이 크고 메모리 상한이 낮다 |
| **스크립팅 백엔드** | Mono 또는 IL2CPP | **IL2CPP 고정** → Managed Code Stripping 이 항상 켜져 있다. 리플렉션·`JsonUtility` 사용 코드는 `link.xml`/`[Preserve]` 필요 ([unity-scripting-gotchas.md](unity-scripting-gotchas.md) §3) |
| **프레임 예산** | thermal 감안 65% | thermal 은 없지만 브라우저 오버헤드가 있다. 60fps 목표면 **~13ms** 를 실사용 예산으로 |
| **프로파일링** | 실기기 Development Build | 브라우저 Development Build + 브라우저 DevTools Performance 탭 병행 |
| **Vsync** | 하드웨어 강제 | 브라우저의 `requestAnimationFrame` 에 묶인다. `targetFrameRate` 가 사실상 무시될 수 있다 |
| **오디오** | 플랫폼 압축 포맷 | WebGL 은 AudioClip 로드 방식이 제한적 — 짧은 SE 는 압축 해제, BGM 은 스트리밍 |

이 게임은 2D URP + 오브젝트 풀 기반 벨트라 GPU 보다 **CPU·GC 쪽이 병목이 될 가능성이 높다.** §2(Memory/GC), §7(UGUI) 을 먼저 본다.

---

## 1. Profiling

- **Editor 프로파일링을 신뢰하지 않는다.** Editor 수치는 상대 비교용. 최종 판단은 **실제 브라우저에서 돌린 Development Build**에서 내린다. 가장 낮은 지원 스펙 환경으로 측정한다.
- **프레임 예산은 타깃의 65%.** 30 fps = 33.33ms, 60 fps = 16.67ms 이지만 실사용 목표는 각각 **~22ms / ~11ms**. WebGL 은 브라우저 오버헤드까지 감안해 **~13ms** 를 60fps 기준선으로 본다.
...
- WebGL 은 브라우저 DevTools 의 Performance / Memory 탭을 병행한다. 초기 로드는 Network 탭에서 `.wasm` / `.data` 크기를 직접 확인.
- (모바일로 확장할 경우) iOS는 Xcode Instruments, Android는 Android Studio Profiler, Arm Mobile Studio, Snapdragon Profiler. 이 경우 기기를 쿨한 상태로 유지 — 과열된 기기의 수치는 거짓말이다.
...
- **WebGL 에서는 브라우저의 `requestAnimationFrame` 이 프레임을 지배한다** — `Application.targetFrameRate` 가 사실상 무시될 수 있다. 프레임 제어보다 **프레임당 작업량 줄이기**로 접근한다.
- **기본 fps는 30** — 60이 꼭 필요하지 않으면 유지. 정적 화면(메뉴 등)에서는 `Application.targetFrameRate`를 더 낮출 수 있다. 이 게임은 벨트가 계속 도는 화면이라 스테이지 씬은 60 기준으로 잡되, 덱빌딩·스테이지 선택 화면은 낮춰도 된다.
...
- **압축 포맷**:
  - **WebGL (이 프로젝트의 타깃): DXT1(불투명) / DXT5(알파)** — 데스크톱 브라우저의 사실상 표준.
  - 모바일로 확장할 경우: ASTC 기본, 구형 iOS 는 PVRTC, 2016년 이전 Android 는 ETC2.
  - 압축이 눈에 거슬리면 **16-bit 비압축**이 32-bit보다 낫다.
- **픽셀 아트 주의** — 이 게임은 일본풍 2D 픽셀 아트다. 스프라이트는 **Filter Mode = Point (no filter)**, **Compression = None 또는 고품질**, **Mip Maps 끄기** 가 기본이다. 블록 압축(DXT)은 픽셀 아트에서 색 뭉개짐이 눈에 띄므로, 아이콘·캐릭터 스프라이트는 Sprite Atlas 로 묶되 압축은 신중히 고른다.
- **Sprite Atlas 는 이 프로젝트에서 특히 중요하다** — 벨트 위에 여러 종류의 초밥이 동시에 돈다. 아틀라스로 묶지 않으면 초밥 종류 수만큼 드로우콜이 늘어난다.
