...
```
1) SVG 작성 (인라인 문자열 또는 Assets/Art/Sprites/UI/<name>.svg)
...
```
Scene.New              → Assets/Level/Scenes/Stage01.unity
...
GameObject.Create      → GameRoot
Component.Add          → /GameRoot, SushiDefense.Belt.SushiBelt
Component.SetField     → sushiPrefab ← Assets/Level/Prefabs/SushiItem.prefab
Component.SetField     → stageConfig ← Assets/Code/Scripts/Runtime.Data/StageConfig/Stage01.asset
Scene.Save
```

### UGUI 프리팹 중첩 (CustomerCard 안에 타겟팅 배지)

```
Prefab.Open              → CustomerCard.prefab
Prefab.InstantiateAsChild → TargetingBadge.prefab as /CustomerCard/TargetingBadge  (원본 링크 유지)
Component.SetRectTransform → /CustomerCard/TargetingBadge, anchor/pivot/position
Prefab.Save
Prefab.Close
```
...
```
Prefab.CreateVariant → sourcePath=SushiItem.prefab, variantPath=SushiItem_Premium.prefab
Prefab.Open          → SushiItem_Premium.prefab
Component.SetField   → Image.sprite ← sushi_toro.png  (오버라이드만 변이)
Prefab.Save
Prefab.Close
```

> 프리팹을 SO 참조와 함께 세팅할 때, **밸런스 수치를 프리팹에 직접 박지 않는다.** 프리팹은 `SushiData` / `CustomerData` 참조만 들고, 수치는 SO 쪽에 있어야 한다 (`CLAUDE.md` §3.1).

## 에이전트가 선제 호출해도 되는 순간

사용자 명령 없어도 판단해서 호출:

- **씬에서 참조되는 프리팹이 `Assets/Level/Prefabs/` 에 없을 때** → `/make-asset ui` 로 임시 블록 프리팹 생성 → 계속 진행. 한 줄 고지: "SushiItem 프리팹 없어서 색 블록으로 임시 생성합니다. 나중에 픽셀 아트 나오면 말씀하세요."
- **심볼/아이콘이 필요한데 이미지 없을 때** (초밥 접시·동전·별·체크·X·화살표·하트 등) → 사용자 요청 기다리지 말고 `/make-asset sprite` 로 **SVG 직접 작성 → PNG 래스터화 → 임포트**. Claude는 단순 도형 SVG를 스스로 그릴 수 있으므로 이미지 요청은 사진·복잡한 일러스트일 때만.
  - 단, 최종 아트는 **일본풍 2D 픽셀 아트**다. SVG placeholder 는 어디까지나 임시이며, 사람이 그린 원본 스프라이트를 덮어쓰지 않는다 (`CLAUDE.md` §7·§9).
