...
왜: 복잡성이 **여러 모듈에 퍼지면** 변경 비용이 기하급수적으로 커진다. 어셈블리(`Runtime.Data`/`Runtime`/`Presentation`)가 격리의 자연 단위.
...
DO
```
# 목표: WebGL 60fps → 실사용 프레임 예산 ~13ms (브라우저 오버헤드 포함)
# 현재 CPU 시간: 16ms, GC.Alloc: 프레임당 4KB (벨트 스폰 경로)
# 스폰 경로를 SushiPool 재사용으로 바꾸면 할당 0, CPU 11ms → 예산 내 복귀
# → 정량 근거로 결정
```
