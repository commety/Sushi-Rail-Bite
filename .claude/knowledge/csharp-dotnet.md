...
Unity는 Mono(또는 IL2CPP) 런타임 위에서 C#을 실행한다. 이 문서는 "왜 이 규칙이 성립하는가"의 근거를 제공한다. 구체적 Unity 성능 규칙은 [unity-webgl-performance.md](unity-webgl-performance.md)에 있다.
...
이 문서의 규칙 중 Unity WebGL 성능에 직결되는 것들 (자세한 건 [unity-webgl-performance.md](unity-webgl-performance.md)):
