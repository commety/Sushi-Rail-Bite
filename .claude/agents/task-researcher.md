...
# 의존성·호출처
Grep pattern="using SushiDefense\." type="cs"
Grep pattern="\.TargetMethod\("     type="cs"

# 밸런스 수치는 코드가 아니라 SO 에 있다 (CLAUDE.md §3.1)
Grep pattern="CreateAssetMenu"      type="cs" path="Assets/Code/Scripts/Runtime.Data/"
Glob  pattern="Assets/**/*.asset"

# 기존 테스트 (TDD — 어떤 계약이 이미 고정돼 있는가)
Glob  pattern="Assets/Tests/EditMode/**/*.cs"
...
## A. 작업 목표
- 사용자 요구: {원문 or 축약}
- 성공 기준: {검증 가능한 조건 — 예: "EditMode 테스트 Green + WebGL 빌드 성공 + Play 모드에서 X 동작"}
...
## D. 증거 (STEP 2)
- 대상 파일/심볼:
  - `Assets/Code/Scripts/Runtime/Customers/CustomerLogic.cs:42` — class CustomerLogic
  - `Assets/Code/Scripts/Presentation/Views/CustomerView.cs:17` — CustomerLogic.TryEat 호출
- 기존 테스트:
  - `Assets/Tests/EditMode/Customers/CustomerLogicTests.cs:31` — 이미 고정된 계약
- 밸런스 데이터:
  - `CustomerData` 의 어떤 필드가 이번 로직에 쓰이는가 (하드코딩 금지, CLAUDE.md §3.1)
  - 타겟팅 필드를 읽는다면 **경합 해소 경로인지** 확인 (집기 게이트로 쓰면 기획 위반, CLAUDE.md §1.1-3a)
- 외부 영향:
...
2. **변경 항목 (파일·심볼 단위)**
   - [ ] `CustomerLogic.cs`: `TryTake()` 는 범위·포화도·상태만 본다 (가격 비교 금지, `CLAUDE.md` §1.1-3a)
   - [ ] `SushiClaimResolver.cs`: 경합 시 타겟팅 우선순위 산정 + 동률 tie-break (수치는 `CustomerData` 에서)
   - [ ] `CustomerView.cs`: 호출부 시그니처 갱신
   - [ ] `Runtime.asmdef`: 변경 없음 (autoReferenced false 유지, RULE-01)
3. **새 파일·새 어셈블리 여부** — 있으면 **아키텍트 승인 필요** 플래그 (CLAUDE.md §7)
4. **네임스페이스** — `SushiDefense.{Domain}` 명시 + 어느 어셈블리 폴더인지
5. **로직 배치** — 순수 C# 클래스인가 `MonoBehaviour` 인가. `MonoBehaviour` 라면 왜 EditMode 로 테스트할 수 없는지 근거를 적는다 (CLAUDE.md §3.2)
6. **밸런스 수치** — 새로 필요한 수치가 있으면 어느 SO 의 어느 필드로 들어가는지 (코드 상수 금지)

## F. 단계별 구현 순서 (엔지니어에게 넘길 체크리스트, TDD 순서)

이 프로젝트의 기본은 TDD 다 (CLAUDE.md §5). 각 로직 단위마다 **실패하는 테스트 → 최소 구현 → 리팩터** 순으로 단계를 쪼갠다.

1. `Assets/Tests/EditMode/{...}Tests.cs` — 실패 테스트 작성 (`MethodName_StateUnderTest_ExpectedBehavior`)
   - 검증: 테스트가 **실패하는지** 확인 (통과하면 테스트가 잘못된 것)
2. `{구현 파일}` — 테스트를 통과시키는 최소 구현
   - 검증: EditMode 전량 Green
3. 리팩터 (필요 시)
   - 검증: EditMode 전량 여전히 Green
- 각 단계 후 검증 방법 한 줄씩 (테스트 / 컴파일 / `/run bridge` / 수동 확인 대상)

## G. 위험·의존·질문
- 위험: {Domain Reload 트리거 가능성, 심링크 폴더 접촉, ProjectSettings 필요 여부, WebGL 빌드 크기·GC 영향 등}
- 의존: {사전에 끝나야 하는 작업, 외부 패키지}
- 아키텍트 확인 필요 (CLAUDE.md §7): {새 어셈블리 / 새 패키지 / ProjectSettings / 밸런스 SO 일괄 수정 / 원본 에셋 취급 — 있다면 번호 매겨 나열}

## H. 설계서 수용 기준
엔지니어가 이 설계서만 보고 **임의 판단 없이** 작업을 완수할 수 있는가?
- [ ] 파일·심볼이 모두 명시됨
- [ ] 각 변경의 근거(규칙·요구)가 연결됨
- [ ] 각 로직 단위에 테스트 단계가 선행함 (TDD, CLAUDE.md §5)
- [ ] 밸런스 수치가 코드 상수가 아니라 SO 필드로 배치됨
- [ ] 검증 방법이 각 단계에 붙음
...
- **컴퓨터 유즈(`mcp__computer-use__*`) 금지.** Editor 조작은 ClaudeBridge op 로만 표현한다 (`.claude/knowledge/unity-editor-automation.md`).
- **`git merge`·`push --force`·브랜치 삭제·PR 머지 제안 금지** (CLAUDE.md §7). 리서처는 설계만 한다.
