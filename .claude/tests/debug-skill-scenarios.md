...
> ⚠️ **아카이브 — Sushi Rail Bite 의 코드가 아니다.**
> 이 문서와 [debug-skill-results.md](debug-skill-results.md) 는 이 템플릿을 **다른 프로젝트(Game2048 샘플)** 에서 검증했을 때의 기록이다.
> 여기 나오는 경로(`Assets/Scripts/_Core/...`)·타입·수치는 **이 저장소의 사실이 아니므로 근거로 인용하지 않는다.**
> 남겨 두는 이유는 스킬의 툴 매트릭스가 어떤 방식으로 검증됐는지 보여주는 **방법론 참고**이기 때문이다.
> Sushi Rail Bite 기준으로 재검증하려면 경로를 `Assets/Code/Scripts/{Runtime,Runtime.Data,Presentation}/`, 네임스페이스를 `SushiDefense.*` 로 바꿔 다시 돌린다.
