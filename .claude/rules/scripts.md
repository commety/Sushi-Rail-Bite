...
- **금지 패턴**:
  ```csharp
  // ❌ 타겟팅을 자격 게이트로 사용 — 멀다고 못 먹게 만든다
  if (Mathf.Abs(sushi.Price - _data.Targeting) > threshold)
      return false;

  // ✅ 자격은 가격을 보지 않는다. 타겟팅은 '무엇을/누가' 를 정할 때만 등장
...
- **한 번 인식된 초밥은 물리적으로 범위를 조금 벗어나도 후보에서 빼지 않는다.** 계산 지연 때문에 눈앞에서 놓치는 그림이 나오면 안 된다. 후보에서 빠지는 것은 소비됨 / 벨트에서 제거됨 / 손님이 자격 상실 세 경우뿐이다.
