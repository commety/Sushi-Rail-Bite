...
### 하네스가 구조적으로 밟지 않는 네 가지 모양

M5 에서 셋이 **빌드 직전까지** 살아남았다. 전부 테스트가 초록이었다.

| 모양 | 왜 안 밟히나 | M5 의 사례 |
|---|---|---|
| **가드가 걸린 `Update`** | 화면이 열려야 도는 코드는 테스트가 화면을 안 열면 실행되지 않는다 | 레거시 `Input` 이 런타임 예외 — Enter·Esc 전멸 |
| **직접 주입으로 우회되는 폴백** | 테스트가 참조를 물려 주면 "비었을 때 찾는" 분기가 죽는다 | `HudLabel.Resolve` · `EffectDirector._pool` |
| **안쪽 진입점만 부르는 테스트** | 바깥 껍데기(입력·좌표 변환)를 지나친다 | `Initialize(…, null, …)` 가 카메라를 지움 |
| **애셋 등록·설정** | 테스트가 경로로 직접 열면 등록 여부를 안 본다 | Build Settings 에 빈 씬만 등록 → 빌드는 성공, 화면은 클리어 색만 |

**공통점: 테스트가 편하려고 만든 우회로가 곧 사각지대다.** 폴백을 두거나 안쪽 진입점을 열었다면, **그 우회로를 타지 않는 테스트 하나**를 같이 남긴다.
...
#### 전량 통과가 곧 공허한 테스트는 아니다

주입했는데 아무도 안 죽으면 **원인이 셋 중 하나**다. 순서대로 배제한다.

| # | 원인 | 확인 | M5 사례 |
|---|---|---|---|
| 1 | **검사 자체가 헛돌았다** | 경로·대상이 맞는지 먼저 본다 | `cd` 가 셸에 남아 참조 검사가 엉뚱한 폴더에서 돌아 "0건" 이 나왔다 — 그대로 믿었으면 지우면 안 될 것을 지웠다 |
| 2 | **주입이 관측 범위 밖이다** | 테스트가 실제로 읽는 데이터인지 본다 | 덱에 없는 초밥의 아이콘을 중복시켰다. 덱 안의 것으로 바꾸니 즉시 잡혔다 |
| 3 | **테스트가 공허하다** | 그때 비로소 테스트를 고친다 | 반례가 잘못된 구현을 배제하지 못했다 |

**1·2 를 배제하기 전에 테스트를 고치지 않는다.** 멀쩡한 테스트를 "보강" 하면 검증은 안 늘고 코드만 는다.

#### 다른 장치가 주입을 가릴 수 있다

검증하려는 계약 위에 **또 다른 억제 장치**가 있으면, 계약을 깨뜨려도 그 장치가 결과를 같게 만든다.

- 구독 중복을 **간격 0.06초짜리 큐**로 검증했더니, 이벤트가 두 번 와도 간격이 두 번째를
  막아 통과했다 → **간격 0 인 큐**로 바꾸니 잡혔다
- 풀 재사용을 **참조 비교**로만 검증했더니, 반납이 안 돼 둘이 떠 있어도 "첫 번째 활성" 이
  여전히 원본이라 통과했다 → **인스턴스 수**를 함께 박아 잡았다
- 「남의 사건을 거른다」를 **관측 대상이 죽어 있는 상태**에서 검증했더니, 옆의 다른 가드가
  어차피 그 값을 버려서 거르는 코드를 통째로 지워도 통과했다 → 그 값이 **살아 있는 상태**로
  옮겨 잡았다
- 같은 것을 **시간 창을 줄여** 결정적으로 만들려 했더니, 그 창이 한 프레임 안에 소진돼
  **또** 통과했다 → 창은 그대로 두고 **다른 경로로 그 상태에 들어가** 잡았다

**억제·중복 제어가 있는 시스템에서 "한 번만 일어났다" 를 볼 때는, 그 제어가 꺼진 조건을 고른다.**

**관측 대상이 시한부 상태이면 배치가 두 번 어긋난다** — 상태가 죽어 있으면 형제 가드가
가리고, 살리려고 창을 줄이면 창이 소진돼 가린다. **창은 건드리지 말고, 그 상태가 살아
있는 다른 진입 경로를 찾는다.**
