...
이 스킬은 **에이전트도 사용자 명령 없이 호출** 가능. 예를 들어 벨트 로직을 짜다 "초밥 아이템 프리팹이 필요" 라는 판단이 서면 `/make-asset` 를 불러 즉시 만든다.
...
| kind | 용도 | 대표 출력 |
|---|---|---|
| `ui` / `prefab` / `button` | UGUI 프리팹 | `Assets/Level/UI/*.prefab` (RectTransform + Image/Text/Button 조합) |
| `particle` / `vfx` | 파티클 이펙트 | `Assets/Level/Prefabs/VFX/*.prefab` (ParticleSystem + Renderer) |
...
>
> **아트 톤:** 일본풍 2D 픽셀 아트, 캐주얼·개그 톤 (README 아트 컨셉). placeholder 라도 이 방향에서 벗어나지 않게 색·형태를 고른다.
...
1. **경로 두 갈래 — 먼저 어느 쪽인지 판단**:
   - **(A) 심볼·아이콘·도형류** (초밥 접시, 젓가락, 동전, 별, 체크, X, 화살표, 하트, 말풍선 등): Claude가 SVG를 **직접 작성**하고 PNG로 래스터화해서 임포트. 이미지 요청 불필요.
...
```
unity_call("Prefab.Open", {path: "Assets/Level/UI/PrimaryButton.prefab"})
...
생성 규칙:
- 파일 경로는 `Assets/Level/UI/<PascalCase>.prefab`
...
```
unity_call("GameObject.Create",   {name: "CoinPop", parentPath: null})
unity_call("Component.Add",       {path: "/CoinPop", type: "UnityEngine.ParticleSystem"})
...
unity_call("Asset.CreatePrefab", {goPath: "/CoinPop", prefabPath: "Assets/Level/Prefabs/VFX/CoinPop.prefab"})
unity_call("GameObject.Delete", {path: "/CoinPop"})
```
...
```
unity_call("GameObject.Create", {name: "CustomerPlaceholder", parentPath: null})

# 몸통 — Cube
unity_call("GameObject.Create", {name: "Body", parentPath: "/CustomerPlaceholder"})
unity_call("Reflection.Invoke", {
  typeName: "UnityEngine.GameObject",
  methodName: "CreatePrimitive",
  argTypes: ["UnityEngine.PrimitiveType"],
  argsJson: ["\"Cube\""]
})  # (이 방식은 CustomerPlaceholder 자식으로 안 감. 실제로는 primitive 생성 후 SetParent가 더 정석)
...
**Step 1. SVG 작성.** 인라인 문자열로 op 인자에 바로 넣거나, `Assets/Art/Sprites/UI/<name>.svg` 에 Write 툴로 먼저 저장한 뒤 경로만 넘긴다.
...
unity_call("Sprite.ImportFromSvg", {
    "svgText":       svg,
    "pngPath":       "Assets/Art/Sprites/UI/heart.png",
    "width":         256,
    "height":        256,
    "pixelsPerUnit": 100,
...
`svgText` 대신 이미 디스크에 있는 .svg 를 쓰고 싶으면 `svgPath: "Assets/Art/Sprites/UI/heart.svg"` 로 넘긴다. `svgText` 가 있으면 그쪽이 우선.
...
# curl -L -o Assets/Art/Sprites/Customers/customer-portrait.png "https://..."
...
r = unity_call("Reflection.Invoke", {
    ...
    "argsJson": ["\"Assets/Art/Sprites/Customers/customer-portrait.png\""]
})
...
- 스크립트를 작성 중인데 Inspector에 붙일 프리팹이 없을 때 (예: `[SerializeField] GameObject sushiItemPrefab;`)
- 씬을 조립 중인데 "초밥 아이템", "손님", "테이블", "버튼" 같은 게임 오브젝트가 참조로 지목됐지만 해당 에셋이 `Assets/` 어디에도 없을 때
- 스테이지 씬을 처음 조립할 때 (벨트·테이블·손님·초밥 placeholder 일괄 생성)

호출 전 사용자에게:
- "SushiItem 프리팹이 필요한데 없습니다. `/make-asset ui` 로 간단한 초밥 프리팹 만들까요? 이미지 있으면 첨부해 주세요 — 없으면 색 블록으로 임시 생성합니다."
