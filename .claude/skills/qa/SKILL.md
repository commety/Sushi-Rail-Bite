...
이 프로젝트는 인디·소규모 팀이다. `/qa` 라는 **별도 QA 패스**는 사용자가 명시적으로 요청했을 때 돌아간다. 다음은 **기본 동작 아님**:
...
> **단, Unity Test Framework 스위트는 예외다.** 이 프로젝트는 TDD 가 기본값이고(`CLAUDE.md` §5), EditMode 테스트 전량 통과는 **커밋 게이트**다(`CLAUDE.md` §8). "테스트 셋업 비용" 을 이유로 스킵하지 않는다.
> `/qa` 는 그 테스트 스위트를 **대체하는 것이 아니라**, 테스트로 못 잡는 것(규칙 준수, 기획 조항 커버리지, 런타임 상태 전이, 성능 예산)을 본다.
...
| [`rules/scripts.md`](../../rules/scripts.md) | `.cs` 경로·컨벤션 규칙 | `.cs` 신규·수정 QA |
| [`rules/asmdef.md`](../../rules/asmdef.md) | 어셈블리 구성·의존 방향 | 어셈블리 변경 QA |
| [`rules/scriptable-object.md`](../../rules/scriptable-object.md) | 데이터 주도 설계 규약 | 밸런스·SO QA |
| [`rules/tests.md`](../../rules/tests.md) | TDD·테스트 배치 규약 | 커밋 게이트 QA |
| [`knowledge/unity-webgl-performance.md`](../../knowledge/unity-webgl-performance.md) | 성능 규약 (WebGL 타깃) | 프레임·메모리·로드시간 QA |
| [`knowledge/csharp-dotnet.md`](../../knowledge/csharp-dotnet.md) | C# 스펙 규약 | 타입·async·이벤트 QA |
| `CLAUDE.md` §1.1 코어 루프 / §2 용어 | 게임 규칙의 1차 출처 | 기능 QA |
| `.claude/domain/*.md` / 기획서 / PR 설명 | 프로젝트 고유 행동 규약 | 기능 QA |

기준이 명시 안 되어 있으면 QA 하지 않는다. 먼저 사용자에게 기준을 묻거나 `/design` 으로 스펙 추출.

**밸런스 수치 자체는 QA 대상이 아니다.** "손님이 너무 빨리 배부르다" 는 버그가 아니라 기획 판단이다. 에이전트는 "현재 값이 SO 의 X 필드에서 오고 있다" 까지 보고하고, 값을 바꾸는 건 사람에게 넘긴다 (`CLAUDE.md` §7).
...
| **RULE-02** 심링크 폴더 파일 신규 금지 | `Bash git status` 로 `Assets/Art/`·`Assets/Audio/` 변경 여부 | — | `git status` 출력 |
...
| 네임스페이스 `SushiDefense.{Domain}` | `Grep "^namespace "` + 파일 경로 매칭 | — | 규약 불일치 파일 0 |
| **밸런스 하드코딩 금지** (`CLAUDE.md` §3.1) | `Grep` 으로 로직 파일의 숫자 리터럴 (`[^\w.]\d+\.?\d*f?`) → SO 필드로 대체 가능한지 판정 | — | 로직 어셈블리의 밸런스성 리터럴 0 |
| **로직/MonoBehaviour 분리** (§3.2) | `Grep ": MonoBehaviour"` 후 해당 파일의 계산 로직 분량 확인 | — | 점수·포화도·타이머 계산이 `MonoBehaviour` 밖에 있음 |
...
```
Component.GetField path="/GameRoot" type="SushiDefense.Scoring.ScoreView"   field="_totalRevenue" → 기대 1200
Component.GetField path="/Tables/Table01" type="SushiDefense.Customers.Customer" field="_saturation" → 기대 3
```
...
요약: 8 기준 중 7 통과, 1 실패 (§3.1 위반 1건). 실패는 수정 후 재검증 필요.
```
...
- **컴퓨터 유즈(`mcp__computer-use__*`) 금지.** Bridge 로 못 보는 거면 op 추가·사용자에게 한 장면 캡처 요청으로 — 좌표 자동화로 우회 금지 (`.claude/knowledge/unity-editor-automation.md`).
...
- **핵심 조항 위주로.** 기획 조항 100개 전수 평가 강요 안 함. **실패 시 아픈 것** 을 우선: 코어 루프(초밥 스폰 → 집기 → 매출), 스테이지 클리어 판정, 덱·런 진행 상태 저장. 연출·사운드·가벼운 UI 문구는 나중.
