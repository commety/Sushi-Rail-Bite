...
- **워크트리 (`agent-0`, `agent-1` 등)에서 작업 중이면**: `Assets/Audio/` 는 심링크라 거기에 쓴 파일은 **워크트리 git 이 안 본다** (메인으로 새어나감). 기능 폴더에 같이 둔다:
  - ✅ `Assets/Code/Scripts/Presentation/UI/SE/blip.wav`
  - ✅ `Assets/Code/Scripts/Presentation/Belt/Audio/sushi-pick.wav`
...
- 게임 로직에서 "초밥 집기", "포화(배부름)", "매출 획득", "스테이지 클리어" 같은 피드백 이벤트가 등장하는데 AudioClip 이 없을 때 → 프리셋+짧은 시퀀스로 임시 SE 생성
- 톤 가이드: 이 게임은 **경쾌한 동양풍/일본풍 + 맛깔난 음식 효과음** 이다 (README 아트 컨셉). 어둡거나 공격적인 프리셋은 피한다
