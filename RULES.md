...
- `Assets/Plugins/`, `Assets/Resources/`, `Assets/StreamingAssets/`, `Assets/AddressableAssetsData/`
- `ProjectSettings/`, `Packages/`

이 목록의 원본은 `scripts/ensure-worktree-setup.sh` 의 `SYMLINK_ASSET_DIRS` / `SYMLINK_ROOT_DIRS` 다.
**스크립트를 고치면 이 목록과 `.claude/rules/parallel-work.md` §1 도 같이 고친다** — 세 곳이 어긋나면 규칙이 오탐·미탐을 낸다.
아직 존재하지 않는 폴더는 심링크되지 않고 조용히 건너뛴다.

`Assets/Level/`(Scenes·Prefabs·UI)은 **의도적으로 심링크하지 않는다.** 능동적으로 편집하는 콘텐츠라
...
sed -n '/^SYMLINK_ASSET_DIRS/,/^)/p' scripts/ensure-worktree-setup.sh
```
