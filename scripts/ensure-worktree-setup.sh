...
#
# 여기 넣는 기준: "여러 워크트리가 공유해야 하고, 워크트리에서 새로 만들 일이 없는" 폴더.
# 존재하지 않는 항목은 link_or_skip 이 조용히 건너뛴다.
#
# 의도적으로 제외한 것:
#   Assets/Level      — 씬·프리팹은 능동적으로 편집하는 콘텐츠다. 심링크하면 워크트리
#                       git 이 새 파일을 못 본다 (parallel-work.md §2). 대신 씬 편집은
#                       한 번에 한 워크트리만 하도록 직렬화한다 (parallel-work.md §3).
#   Assets/Code       — 브랜치별로 달라야 하는 소스. 당연히 제외.
#   Assets/Tests      — 위와 동일.
SYMLINK_ASSET_DIRS=(
  "Assets/Art"
  "Assets/Audio"
  "Assets/Settings"          # URP 렌더 파이프라인 애셋 — 전 워크트리 공유
  "Assets/Plugins"
  "Assets/Resources"
