...
### 밸런스 수치 (§7-2 — **결정 완료: 그대로 둔다**)

> **아키텍트 결정 (2026-08-09)**: 한도를 올리지 않는다. 「자리가 남아도 못 앉힌다」가 의도다 → README **D11**.
> 아래는 그 결정의 근거가 된 실측이며, **step-09 는 이것을 다시 묻지 않는다.**
>
> | 스테이지 | 자리 | 한도 | 초기예산 | 먹보를 넣는 최선 조합 (예산 준수) | 쓰는 자리 |
> |---|---|---|---|---|---|
> | Stage01 | 3 | 3 | 60 | 먹보×1 — 인구 2/3, 비용 60/60 | **1/3** |
> | Stage02 | 4 | 4 | 80 | 기본×1 + 먹보×1 — 인구 3/4, 비용 80/80 | 2/4 |
> | Stage03 | 4 | 4 | 100 | 기본×2 + 먹보×1 — 인구 4/4, 비용 100/100 | 3/4 |
>
> **초반에는 한도가 아니라 예산이 먼저 막는다.** 한도가 실제로 물리는 것은 매출이 쌓인 뒤다.

<details>
<summary>착수 당시의 원래 지시 (보존)</summary>
