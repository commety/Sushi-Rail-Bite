...
- **D11. 한도를 올리지 않는다. 「자리가 남아도 못 앉힌다」가 의도다.** *(step-03 에서 실측 후 확정)*
  자리 수와 한도가 같은 값(3/3, 4/4, 4/4)이라 먹보를 쓰면 자리 하나가 놀게 되는데, **그 손실이 곧 먹보의 대가다.** 실측해 보니 초반에는 한도가 아니라 **초기 예산이 먼저 막는다** — Stage01 에서 먹보(비용 60)를 뽑으면 예산 60 이 정확히 소진돼 자리 셋 중 하나만 쓴다. 인구수 한도(3 중 2)는 그때 아직 여유가 있다. 한도가 실제로 물리는 것은 매출이 쌓여 재화가 붙은 뒤(먹보+기본 = 비용 80, 인구 3/3)이며, 그 시점이 «세 번째 자리를 버리고 먹보를 유지할 것인가» 를 묻는 자리다.
...
| 2 | ~~`Stage01~03.asset` 의 `_maxPlacedCustomers` 재조정~~ → **그대로 둔다 (결정됨)**. 아래 D11 | step-03 — 완료 |
