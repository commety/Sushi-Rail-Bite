...
> **step-03 에서 정정됨.** 처음에는 "조율자 → 시계 → 판정" 이라는 3단 순서 전체가 계약이라고 적었는데, 주입 실험 결과 **조율자와 시계의 상대 순서는 관측되지 않았다** — 둘을 바꿔 넣어도 357개가 전부 통과했다. 조율자는 시계를 읽지 않고 시계는 매출을 읽지 않으므로, 판정이 마지막에 두 최종값을 읽는 한 결과가 같다.

실제로 지켜야 하는 것은 하나다: **판정이 조율자 틱보다 뒤에 온다.** 앞서 판정하면 이번 프레임에 입에 들어간 초밥의 매출이 빠진 채로 만료가 걸려 "먹었는데 실패" 가 난다. 시계를 먼저 흘리고 판정한 뒤 조율자를 굴리는 구현을 주입하자 `Tick_TargetReachedOnTheExpiringFrame_Clears` 가 매출 0 으로 실패했다.
