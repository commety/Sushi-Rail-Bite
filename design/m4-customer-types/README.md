...
| 질문 | 결정 |
|---|---|
| 세 유형의 대역 배치 | 착수 시 **3분할, 겹침 없음** 으로 정했으나 **실측에서 뒤집혔다** — 아래 "실행 중 바뀐 것" 참조 |
...
|---|---|---|---|
| 먹보 (신규) | **100~150** | 50 | 신규 |
| 기본 | **100~300** | 200 | **변경 없음** (3분할 시도 후 되돌림) |
| 소식 | 250~320 | 70 | 변경 없음 |

> **실행 중 바뀐 것.** 착수 시 기본을 150~250 으로 좁히기로 했으나, step-10 실측에서
> **스테이지 1 이 클리어 불가**가 됐다 (매출 4990 / 목표 5000, 소비율 96%→78%).
> 원인은 경합이 아니라 **타이밍**이다 — 대역 밖 초밥은 이탈 직전까지 유예되므로, 경합
> 상대가 하나도 없는 스테이지 1 에서도 손님이 그만큼 논다. 아래 "겹치면 넓은 대역이
> 밀린다" 는 여전히 맞지만, **겹치지 않게 만드는 비용이 그보다 크다.**
> 근거는 [`customer-kinds.md`](../../.claude/domain/customer-kinds.md) §3.
...
| 가격 | 초밥 | 대역 안인 손님 | 겹치면 이기는 쪽 |
|---|---|---|---|
| 120 | 한치 | 먹보 · 기본 | 먹보 (폭 50) |
| 130 | 달걀 (보상) | 먹보 · 기본 | 먹보 |
| 150 | 연어 · 광어 | 먹보 · 기본 | 먹보 |
| 190 | 방어 | 기본 | — 기본이 독점 |
| 250 | 참치 (보상) | 기본 · 소식 | 소식 (폭 70) |
| 300 | 장어 | 기본 · 소식 | 소식 |
| 400 | 성게 (보상) | **아무도 없음** | 소식이 거리 80 으로 가장 가깝다 |

기본은 **전 구간을 즉시 처리**하되, 전문가가 앉아 있는 가격은 내준다. 190 만이 기본의 독점 구간이다.
...
**step-10 실측 (기본 손님만, 최대 인원)** — 클리어 시점에 판이 멈추므로 벨트 총액은 예측보다 작다.

| | 인원 | 벨트 총액 | 매출 | 소비율 | 목표 | 결과 | 여유 |
|---|---|---|---|---|---|---|---|
| Stage 01 | 3/3 | 5220 | **5030** | 96.4% | 5000 | Cleared | +30 |
| Stage 02 | 4/4 | 6850 | **6280** | 91.7% | 6200 | Cleared | +80 |
| Stage 03 | 4/4 | 8070 | **7610** | 94.3% | 7500 | Cleared | +110 |

세 판 모두 **기본 손님만으로** 닿는다. 보상으로 전문가를 얻으면 더 쉬워지므로 목표는 그대로 둔다.
여유가 30~110 뿐이라 빡빡한 공선이며, **초밥을 덱에 추가하거나 손님 값을 만지면 반드시 재측정한다.**
