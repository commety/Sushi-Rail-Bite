...
| 항목 | 결과 |
|---|---|
| **한글 표시** | **두부 0개** — `매출 0/5000` · `남은 시간 60` · `영입 재화 40` · `손님 1/3` · `대기 0` · `배치 예정 기본` |
| 예외 | **0건** |
| 배경 · 벨트 레일 | 렌더됨 |
| 초밥 | 접시 티어 색으로 구분되어 벨트 위에 |
| HUD 레이아웃 | 겹침 없음 (첫 조립에서는 겹쳤다 — 줄 간격을 폰트 크기보다 크게 잡아 해결) |
| **클릭 배치** | 동작 — `손님 0/3 → 1/3`, `영입 재화 60 → 40` |
| 초기 로드 | **42.45 MB** (무압축 — wasm 29.78 / data 12.65 / js 0.33) |

경고 하나: `Shader 'Hidden/URP/Edge Adaptive Spatial Upsampling' is not supported` → 포스트
프로세싱 패스 미실행. 우리는 쓰지 않으므로 무해하다.

### 이 환경에서 확인하지 못한 것

- **먹힘 → 소리·이펙트의 실제 관측.** 자동화 브라우저에서 캔버스가 포커스를 못 받아 Unity 가
  심하게 스로틀한다 (5초 실시간에 게임 1초). 배선은 `Play_Scene_EatenSushi_ReachesAudioAndEffects`
  가 덮지만 **귀로 듣는 확인은 사람이 해야 한다**
- **드로우콜** — 프로파일러가 필요하다. M8 로 넘긴다
- **창 크기 변경 시 HUD 유지** — `CanvasScaler` 는 걸었지만 실제 리사이즈는 안 해 봤다

### 빌드 직전까지 못 잡은 사고 셋

세 가지 모두 **테스트가 초록인 채로** 남아 있다가 브라우저에서 드러났다.

| 사고 | 왜 테스트가 못 잡았나 |
|---|---|
| Build Settings 에 빈 씬만 등록 | 씬 테스트는 경로로 직접 연다 (RULE-06 회피 때문) |
| `Initialize(..., null, ...)` 가 카메라 참조를 지움 | 테스트가 `ClickAt` 을 직접 불러 카메라 경로를 지나친다 |
| 레거시 `Input` 이 예외를 던짐 | 두 뷰의 `Update` 는 화면이 열려 있을 때만 입력을 읽는다 |

**셋 다 "코드도 테스트도 멀쩡한데 재생만 죽는" 형태다** (`tests.md` §1). 앞의 둘은 회귀
테스트를 붙였고, 세 번째는 `InputBackendTests` 가 컴파일 시점에 고정한다.
