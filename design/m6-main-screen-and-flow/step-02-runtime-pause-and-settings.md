...
| 주입 | 예측 |
|---|---|
| `PauseState` 의 중복 억제 제거 (`Changed` 를 무조건 발행) | `Pause_Twice_RaisesChangedOnce` |
| `GameSettings.Load` 가 `Set*` 대신 필드에 직접 대입 | `Load_CorruptedVolume_PassesThroughClamp` |
| `FilledCells` 의 `ceil` → `floor` | `FilledCells_OneEaten_ReturnsAtLeastOne` |
| `VisibleCells` 에서 `min` 제거 | `FilledCells_MaxAboveCapacity_ScalesDown` |

**예측은 가설이다.** 실제로 돌려 보고 빗나가면 표를 실측으로 정정한다 — M4 에서 세 번 빗나갔다.

### 완료 판정

- [ ] `grep -c "UnityEngine" Assets/Code/Scripts/Runtime/Settings/*.cs Assets/Code/Scripts/Runtime/Stages/PauseState.cs Assets/Code/Scripts/Runtime/Customers/SaturationGauge.cs` — 전부 **0**
- [ ] EditMode Green — `./tests/run-tests.sh`
- [ ] 주입 4건을 실제로 넣어 보고 표를 정정했다
