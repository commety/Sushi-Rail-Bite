...
└── Ops/
    ...
    └── SvgOps.cs               # Sprite.ImportFromSvg (Unity Vector Graphics 로 SVG 렌더 → PNG → Sprite)
```
...
- `Unity -batchmode -quit -executeMethod Editor.ClaudeBridge.ClaudeBridgeBatch.Run` 호출
...
> **`-nographics` 는 쓰지 않는다.** `Sprite.ImportFromSvg` 처럼 `GL`/`RenderTexture`/`Graphics.DrawMeshNow` 가 필요한 op 은 `-nographics` 에서 no-op 으로 무음 실패한다 (빈 PNG 생성). macOS/Windows 에서는 `-batchmode` 만으로도 숨겨진 Metal/D3D 컨텍스트가 떠서 완전 비대화형으로 동작한다. 순수 Linux 헤드리스 CI 에서 돌릴 거면 Xvfb 를 띄우거나 렌더가 필요 없는 op 만 사용하도록 가드해야 한다.
...
| `Sprite.ImportFromSvg` | `{svgText?, svgPath?, pngPath, width?=256, height?=256, pixelsPerUnit?=100, filterMode?="Bilinear", compression?="None", antiAliasing?=4, saveSvgSource?=false}` | `{pngPath, svgPath?, width, height, pixelsPerUnit}` | Unity Vector Graphics 로 SVG → PNG → Sprite 임포트. **외부 바이너리 불필요** (ImageMagick/rsvg 대체). `svgText`/`svgPath` 중 하나 필수. `pngPath` 는 `Assets/...` 하위. 패키지 `com.unity.vectorgraphics@3.0.0-preview.7+` 필요 (manifest.json 기본 포함). **그래픽 컨텍스트 요구** (`-nographics` 불가) — bridge-run.sh 기본 설정으로 동작. |
